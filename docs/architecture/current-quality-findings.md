...
- A subsequent prerender regression was traced to a browser-only `onClick` handler added to the shared Server Component; removing it restored `/en` static generation. The header remains intentionally server-rendered and must not receive browser event handlers or access `window`/`document`.
