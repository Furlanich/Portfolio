...
Initiative 4's Projects/Evidence decisions also fit the accepted boundary. [`PLAN-PROJECTS-EVIDENCE-EXPERIENCE`](docs/plans/active/projects-evidence-experience.md) selects a typed static public manifest, route-owned localized public copy, shared locale-agnostic Server Components, and conditional static detail generation. Internal permission and evidence audits remain Markdown in `docs/product/projects/` and are never parsed or mirrored into public application data. Task 1 established the empty fail-closed contract without public behavior. Task 2 now populates that manifest and adds the static bilingual index/navigation. Three item records satisfy the existing detail gate with paired bilingual content, approved repository evidence links, and labeled conceptual visuals; Task 3 may add their static detail routes within the same architecture. No CMS, backend, project API, database, filter/search subsystem, media service, hosting change, RFC, or new ADR is required.
