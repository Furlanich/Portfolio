    ...
    >
      <div className="grid gap-6 md:grid-cols-2 lg:grid-cols-3">
        {projects.map((project) => {
          ...
          const hasSocials = (project.socialMedia?.length ?? 0) > 0;
          const socials = project.socialMedia ?? [];
          ...
          return (
            ...
            >
              ...
              <div
                className={`space-y-4 p-6 transition-[padding-bottom] duration-200 ease-out ${
                  hasSocials ? 'group-hover:pb-20 group-focus-within:pb-20' : ''
                }`}
              >
              ...
              {hasSocials ? (
                <div className="invisible absolute bottom-6 left-6 flex translate-y-2 items-center gap-3 opacity-0 transition-all duration-200 ease-out group-hover:visible group-hover:translate-y-0 group-hover:opacity-100 group-focus-within:visible group-focus-within:translate-y-0 group-focus-within:opacity-100">
                  {socials.map((social) => (
                    <a
                      key={social.url}
                      href={social.url}
